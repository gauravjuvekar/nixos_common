...
let
  ...
  firefox_addons = inputs.firefox-addons.outputs.packages.${pkgs.stdenv.hostPlatform.system};
in
{
  config =
    lib.mkIf hostinfo.isLocalGraphical
      {
        "home-manager" = {
          programs.firefox = {
            enable = true;
            profiles."Personal" = {
              isDefault = domain_is_personal;
              id = 0;
              settings = {
                "network.trr.mode" = if domain_is_personal then 3 else 0;
              ...
              extensions.packages = with firefox_addons; [
